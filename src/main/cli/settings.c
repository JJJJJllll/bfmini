...
// PG_PID_PROFILE
    ...
    { PARAM_NAME_PIDSUM_LIMIT,      VAR_UINT16 | PROFILE_VALUE, .config.minmaxUnsigned = { PIDSUM_LIMIT_MIN, PIDSUM_LIMIT_MAX }, PG_PID_PROFILE, offsetof(pidProfile_t, pidSumLimit) }, // PID SUM 限幅，pitch roll一个100-1000，yaw自己 240730 by JJJJJllll 但实际仅仅容许50% 所以不用改1000，要在cli改pipdsumlimit 然后别忘了save
    ...
    { PARAM_NAME_ANGLE_LIMIT,           VAR_UINT8  | PROFILE_VALUE, .config.minmaxUnsigned = { 10, 85 }, PG_PID_PROFILE, offsetof(pidProfile_t, angle_limit) }, // 这里对angleLimit限幅了，0-85，但我设为85和180时差别非常明显
