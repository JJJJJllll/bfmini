...
// PG_PID_PROFILE
    ...
    { PARAM_NAME_PIDSUM_LIMIT,      VAR_UINT16 | PROFILE_VALUE, .config.minmaxUnsigned = { PIDSUM_LIMIT_MIN, PIDSUM_LIMIT_MAX }, PG_PID_PROFILE, offsetof(pidProfile_t, pidSumLimit) }, // PID SUM 限幅，pitch roll一个100-1000，yaw自己 240730 jsl 但实际仅仅容许50% 所以不用改1000，要在cli改pipdsumlimit 然后别忘了save
